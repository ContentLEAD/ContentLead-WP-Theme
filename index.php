	...
	<div id="news">
		...
		<?php $categories = get_categories();
			...
			<?php foreach($categories as $category) : 
					  if ($counter == $cat_count_half) : ?>
					  		</div> <!-- End .left-col -->
					  		<div class="right-col">
					  <?php endif; $counter++; ?>
				
				<?php $news_cat_query = new WP_Query(array('cat' => $category->cat_ID, 'post__not_in' => $latest_news_post_ids, 'posts_per_page' => $category_post_show));
					if ($news_cat_query->have_posts()) : ?>
						<section class="news-category" id="category-<?php echo $category->cat_ID; ?>">
							<h3 class="category-title"><a href="<?php echo get_category_link($category->cat_ID)?>" title="Category <?php echo $category->cat_name; ?>"><?php echo $category->cat_name; ?></a></h3> 
								...
								<?php endwhile; wp_reset_postdata(); // Reset Post Data ?>
